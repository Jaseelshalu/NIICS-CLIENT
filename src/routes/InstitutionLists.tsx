...
import { Plus, Edit, Trash2, ChevronDown, ChevronUp, Search, SortAsc, SortDesc, Filter } from "lucide-react"
...
type SortConfig = {
    key: keyof InstitutionList
    direction: 'asc' | 'desc'
}

const sampleCenters: InstitutionList[] = [
    { id: '1', code: 'EC001', name: 'City Central', address: '123 Main St, Cityville', contact: '+1 234-567-8901', seat: 100 },
    { id: '2', code: 'EC002', name: 'Suburban Institute', address: '456 Oak Rd, Suburbtown', contact: '+1 234-567-8902', seat: 200 },
    { id: '3', code: 'EC003', name: 'Rural College', address: '789 Farm Lane, Countryside', contact: '+1 234-567-8903', seat: 50 },
]

export default function InstitutionListsPage() {
    const [centers, setCenters] = useState<InstitutionList[]>(sampleCenters)
    const [isCreateModalOpen, setIsCreateModalOpen] = useState(false)
    const [isEditModalOpen, setIsEditModalOpen] = useState(false)
    const [currentCenter, setCurrentCenter] = useState<InstitutionList | null>(null)
    const [searchTerm, setSearchTerm] = useState('')
    const [sortConfig, setSortConfig] = useState<SortConfig>({ key: 'code', direction: 'asc' })
    const [filters, setFilters] = useState<Partial<Record<keyof InstitutionList, string>>>({})


    const handleCreateCenter = (center: Omit<InstitutionList, 'id'>) => {
    ...
    const handleEditCenter = (center: InstitutionList) => {
    ...
    const handleSort = (key: keyof InstitutionList, direction: 'asc' | 'desc') => {
        setSortConfig({ key, direction })
    }

    const handleFilter = (key: keyof InstitutionList, value: string) => {
        ...
        return centers
            .filter(center =>
                Object.entries(filters).every(([key, value]) =>
                    center[key as keyof InstitutionList].toString().toLowerCase().includes(value.toLowerCase())
                ) &&
        ...
        <div className="container mx-auto p-4 space-y-8">
            ...
            <Card className="bg-gradient-to-br from-background to-secondary">
                {/* <CardHeader className="bg-background/50 backdrop-blur-sm">
                    <CardTitle className="text-2xl font-bold text-primary">Institution List</CardTitle>
                </CardHeader> */}
                <CardContent>
                    <Table>
                        <TableHeader>
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="bg-background/95 backdrop-blur-sm border-primary/20">
                                                    <DropdownMenuItem onClick={() => handleSort(key as keyof InstitutionList, 'asc')} className="flex items-center">
                                                        <SortAsc className="mr-2 h-4 w-4" /> Sort Ascending
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem onClick={() => handleSort(key as keyof InstitutionList, 'desc')} className="flex items-center">
                                                    ...
                                                    <DropdownMenuItem onSelect={(e) => e.preventDefault()}>
                                                        <div className="relative w-full">
                                                            <Input
                                                                ...
                                                                value={filters[key as keyof InstitutionList] || ''}
                                                                onChange={(e) => handleFilter(key as keyof InstitutionList, e.target.value)}
                            ...
                            <AnimatePresence>
                                {filteredAndSortedCenters.map((center) => (
                                    ...
                                    >
                                        ...
                                        <TableCell>{center.seat}</TableCell>
                                        <TableCell>
...
function CenterForm({ onSubmit, initialData }: {
    onSubmit: (center: InstitutionList) => void
    initialData?: InstitutionList
}) {
    const [formData, setFormData] = useState<InstitutionList>(
        initialData || { id: '', code: '', name: '', address: '', contact: '', seat: 0 }
    )
